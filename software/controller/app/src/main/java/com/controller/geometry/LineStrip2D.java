...
public class LineStrip2D {
    ...
    private RingBuffer<Point2D<Float>> points = new RingBuffer<Point2D<Float>>(4560);
    ...
    public void draw(float[] mvpMatrix, float[] color) {
        ...
        synchronized ( lock ) {
            ...
            for (int i = 0; i < coordsSize; i++) {
                vertexBuffer.put(-points.get(i).getX() + points.get(0).getX() + 2.3f);
                vertexBuffer.put(points.get(i).getY());
